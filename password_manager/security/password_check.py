...
def get_pass():
    ...
    if not "passwords" in os.listdir():
        pass_path_R = lambda full_path: full_path.split("\\") if os.name == 'nt' else full_path.split("/")
    ...
    status = lambda code: "Success!" if code == 200 else "Unauthorised!"
    ...
    while True:
        try:
            user = choice.Menu(["Save a password", "Read a password", "Back to explorer"],
                               title="Choose an option:").ask()
